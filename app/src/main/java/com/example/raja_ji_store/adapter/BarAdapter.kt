...
import com.example.raja_ji_store.model.Bardata
import com.example.raja_ji_store.R

class BarAdapter(var nList:List<Bardata>,private val context: Context): RecyclerView.Adapter<BarAdapter.barholder>() {
    inner class barholder(itemView: View): RecyclerView.ViewHolder(itemView){
    ...
    fun setFilteredList(nList: List<Bardata>){
        this.nList=nList
        notifyDataSetChanged()
    }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): barholder {
        val view= LayoutInflater.from(parent.context).inflate(R.layout.baritem,parent,false)
        return barholder(view)
    }
