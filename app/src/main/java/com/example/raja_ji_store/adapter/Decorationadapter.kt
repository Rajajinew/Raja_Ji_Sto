...
import com.example.raja_ji_store.model.decorationdata

class decorationadapter(var nList:List<decorationdata>,private val context: Context): RecyclerView.Adapter<decorationadapter.decorationItemHolder>() {
    inner class decorationItemHolder(itemView: View): RecyclerView.ViewHolder(itemView){
    ...
    fun setFilteredList(nList: List<decorationdata>){
        this.nList=nList
        notifyDataSetChanged()
    }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): decorationItemHolder {
        val view= LayoutInflater.from(parent.context).inflate(R.layout.decorationitem,parent,false)

        return decorationItemHolder(view)
    }
    ...
    override fun onBindViewHolder(holder: decorationItemHolder, position: Int) {
