...
import com.example.raja_ji_store.adapter.danceadapter
...
class DancerActivity : AppCompatActivity() {
    ...
    private var nList = ArrayList<Dancerdata>()
    private lateinit var adapter: danceadapter
    ...
    private fun filterList(query:String?) {
        if (query != null) {
            val filteredList = ArrayList<Dancerdata>()
            for (i in nList) {
    ...
    private fun retrivedata() {
        ...
        val menuRef =database.reference.child("Dancer")
        menuRef.addListenerForSingleValueEvent(
            object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    nList.clear()
                    for (Snapshot in snapshot.children) {
                        val carpetd = Snapshot.getValue(Dancerdata::class.java)
                        if (carpetd != null) {
                            nList.add(carpetd)
    ...
    private fun setAdapter() {
        ...
        recyclerView.layoutManager = LinearLayoutManager(this)
        recyclerView.adapter= danceadapter(nList,this)
    }
}