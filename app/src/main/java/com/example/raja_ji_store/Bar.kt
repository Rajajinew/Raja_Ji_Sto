...
import com.example.raja_ji_store.adapter.BarAdapter
...
class Bar: AppCompatActivity() {
    ...
    private var nList = ArrayList<Bardata>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       retrivedata()
        ...
        recyclerView.layoutManager = LinearLayoutManager(this)
        recyclerView.adapter=BarAdapter(nList,this)
    ...
    private fun filterList(query:String?) {
        if (query != null) {
            val filteredList = ArrayList<Bardata>()
            for (i in nList) {
    ...
    private fun retrivedata() {
        ...
        val menuRef =database.reference.child("Bar")
        menuRef.addListenerForSingleValueEvent(
            object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    nList.clear()
                    for (Snapshot in snapshot.children) {
                        val carpetd = Snapshot.getValue(Bardata::class.java)
                        if (carpetd != null) {
                            nList.add(carpetd)
