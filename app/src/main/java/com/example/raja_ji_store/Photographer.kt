...
import com.example.raja_ji_store.adapter.photographerAdapter
...
class Photographer: AppCompatActivity() {
    private lateinit var recyclerView: RecyclerView
    private lateinit var searchView: SearchView
    private lateinit var backbutton: ImageButton
    private var nList = ArrayList<photodata>()
    ...
    private fun filterList(query:String?) {
        if (query != null) {
            val filteredList = ArrayList<photodata>()
            for (i in nList) {
    ...
    private fun retrivedata() {
        ...
        val menuRef =database.reference.child("Photographer")
        menuRef.addListenerForSingleValueEvent(
            object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    nList.clear()
                    for (Snapshot in snapshot.children) {
                        val carpetd = Snapshot.getValue(photodata::class.java)
                        if (carpetd != null) {
                            nList.add(carpetd)
    ...
    private fun setAdapter() {
        ...
        recyclerView.layoutManager = LinearLayoutManager(this)
        recyclerView.adapter= photographerAdapter(nList,this)
    }
