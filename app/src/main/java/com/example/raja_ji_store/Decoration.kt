...
import com.example.raja_ji_store.adapter.decorationadapter
...
class Decoration : AppCompatActivity() {
    ...
    private var nList = ArrayList<decorationdata>()
    private lateinit var adapter: decorationadapter
    ...
    private fun filterList(query:String?) {
        if (query != null) {
            val filteredList = ArrayList<decorationdata>()
            for (i in nList) {
    ...
    private fun retrivedata() {
        ...
        val menuRef =database.reference.child("Decoration")
        menuRef.addListenerForSingleValueEvent(
            object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    nList.clear()
                    for (Snapshot in snapshot.children) {
                        val carpetd = Snapshot.getValue(decorationdata::class.java)
                        if (carpetd != null) {
                            nList.add(carpetd)
    ...
    private fun setAdapter() {
        ...
        recyclerView.layoutManager = LinearLayoutManager(this)
        recyclerView.adapter= decorationadapter(nList,this)
    }
