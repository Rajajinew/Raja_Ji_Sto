...
import com.example.raja_ji_store.adapter.gromAdapter
import com.example.raja_ji_store.model.carbetdata
import com.example.raja_ji_store.model.groomdata
...
class Gmakeup: AppCompatActivity() {
    private lateinit var recyclerView: RecyclerView
    private lateinit var searchView: SearchView
    private lateinit var backbutton: ImageButton
    private var nList = ArrayList<groomdata>()
    private lateinit var database: FirebaseDatabase
    private lateinit var adapter: gromAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_gmakeup)
    ...
    private fun filterList(query:String?) {
        if (query != null) {
            val filteredList = ArrayList<groomdata>()
            for (i in nList) {
    ...
    private fun retrivedata() {
        ...
        val menuRef =database.reference.child("GMakeup")
        menuRef.addListenerForSingleValueEvent(
            object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    nList.clear()
                    for (Snapshot in snapshot.children) {
                        val carpetd = Snapshot.getValue(groomdata::class.java)
                        if (carpetd != null) {
                            nList.add(carpetd)
    ...
    private fun setAdapter() {
        ...
        recyclerView.layoutManager = LinearLayoutManager(this)
        recyclerView.adapter= gromAdapter(nList,this)
    }
