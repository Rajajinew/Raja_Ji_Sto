...
import com.example.raja_ji_store.adapter.hoteladapter
import com.example.raja_ji_store.model.carbetdata
import com.example.raja_ji_store.model.hoteldata
...
class HotelActivity: AppCompatActivity() {
    ...
    private var nList = ArrayList<hoteldata>()
...
retrivedata()
    ...
    private fun filterList(query:String?) {
        if (query != null) {
            val filteredList = ArrayList<hoteldata>()
            for (i in nList) {
    ...
    private fun retrivedata() {
        ...
        val menuRef =database.reference.child("Hotel")
        menuRef.addListenerForSingleValueEvent(
            object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    nList.clear()
                    for (Snapshot in snapshot.children) {
                        val carpetd = Snapshot.getValue(hoteldata::class.java)
                        if (carpetd != null) {
                            nList.add(carpetd)
    ...
    private fun setAdapter() {
        ...
        recyclerView.layoutManager = LinearLayoutManager(this)
        recyclerView.adapter= hoteladapter(nList,this)
    }
